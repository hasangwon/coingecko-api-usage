...
const Detail = () => {
  const { coin, loading, error, currency, coinIds, selectCurrency, onScrapClick } = useDetail();
  ...
  return error ? (
    ...
    <div className="flex flex-col w-full h-full p-8 min-w-[375px]">
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center">
          <button className="w-[24px] mr-4" onClick={() => onScrapClick(coin.id)}>
            {coinIds && coinIds.includes(coin.id) ? <img src="/fill_star.svg" className="w-[24px] m-1" alt="fill star" /> : <img src="/empty_star.svg" className="w-[24px] m-1" alt="empty star" />}
          </button>
      ...
      <hr />
      {isShownDescription && <p className="mt-4">{coin.description.ko}</p>}
    </div>
  ) : (
    <div>새로고침</div>
  );
